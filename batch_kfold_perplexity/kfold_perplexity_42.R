...
perplexity_results_42 <- foreach (i = 1:num_folds, .combine = data.frame) %dopar% {
  tm_training <- get(paste("training_", i, sep = ""))
  tm_test <- get(paste("test_", i, sep = ""))
  
  train_model <- LDA(tm_training, k = 42, method = "Gibbs", control = controls_tm)
  perplexity <- perplexity(train_model, newdata = tm_test, method = "Gibbs", control = controls_tm)
  
}

save.image("kfold_perplexity_42.RData")